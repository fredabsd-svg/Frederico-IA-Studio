...
| `frontend/src/hooks/useDevProjects.test.js` (acréscimos) | A autorização de publicação sobrevive a reabrir a conversa, e **invalida** ao trocar de branch, de repositório ou de destino; não vale para vínculo de pasta do PC; ações desconhecidas são descartadas antes de sair do navegador; projeto antigo sem o campo `permissions` carrega sem quebrar |
