...
| `backend/src/agent.pipelineRuns.test.js` | **Pipeline fail-closed e multiusuário** (PostgreSQL): reserva preserva objetivo/opções/runId; índice ativo vira conflito explícito; segundo run não nasce; leitura, atualização, conclusão e stop exigem e aplicam `user_id`; stop pós-restart encerra o coordenador do dono |
| `backend/src/subagents.budget.test.js` + `frontend/src/companionMode.test.js` | Regressão explícita de budget acima de duas etapas; estados Ativo/Silencioso/Desligado do Nino e patches que removem proatividade/animação sem ligar voz ao reativar |
...
| `sandbox/*_test.py` (kits 2.1, capa pelo tamanho) | Documento curto dos presets gerencial, parecer e proposta **não ganha capa**: abre com cabeçalho na página 1, sem quebra de página, sem faixa de fechamento automática e — medido no PDF gêmeo — cabe numa página; com 4+ seções volta a ter capa e sumário; `capa=True/False` vence o limiar; carta não ganha cabeçalho. No PDF, o cabeçalho da coluna numérica fica à direita e o TOTAL em negrito. `kits_test.EstruturaTests` trava `decide_capa`/`decide_fechamento` |
| `backend/src/seed.docpro.test.js` | **Com Postgres**: a persona do assistente "Documentos profissionais" gravada no v4.2 (`v14.txt`) migra para a atual, com ou sem quebra de linha no fim; prompt personalizado não é tocado. O hash da persona em `qaFixes.test.js` acusa mudança feita sem arquivar a anterior |
