...
// runner só existe a partir do Node 22. No Node 20 — a versão da imagem de
// PRODUÇÃO (Dockerfile: node:20-slim) — o argumento entre aspas chega literal e
// o runner responde `Could not find '.../src/**/*.test.js'`, com código 1.
// Rodando localmente em Node 22 isso passava despercebido; o job de matriz do
// CI em Node 20 é que expôs. Aqui a descoberta é feita em JS, então o comando
// funciona igual em qualquer versão suportada.
//
// Uso:
//   node scripts/run-tests.mjs                 # tudo em src/
//   node scripts/run-tests.mjs src/memory      # só um subdiretório
