...
// Descarta sandboxes ativos para que novos mounts entrem em vigor na próxima
// execução (usado quando as pastas do PC mudam).
//
// ESCOPO POR USUÁRIO: com `userId`, só os containers DAQUELE usuário caem. Antes
// derrubava os de TODO MUNDO — um usuário mexendo nas próprias pastas matava a
// execução em andamento dos outros. Sem `userId` (boot/manutenção), derruba todos.
export async function destroyAllSandboxes(userId = null) {
  const owner = userId == null ? null : String(userId);
  for (const [id, entry] of [...sessions]) {
    if (owner !== null && String(entry.userId ?? '') !== owner) continue;
    sessions.delete(id);
