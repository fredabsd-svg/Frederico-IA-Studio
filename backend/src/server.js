...
async function seedDocProAssistant(userId) {
  try {
    ...
    } else if (!String(exists.system_prompt || '').trim() || exists.system_prompt === DOCPRO_PROMPT_LEGACY || exists.system_prompt === DOCPRO_PROMPT_V2 || exists.system_prompt === DOCPRO_PROMPT_V3 || exists.system_prompt === DOCPRO_PROMPT_V4 || exists.system_prompt === DOCPRO_PROMPT_V5) {
      // Migra dos prompts padrão anteriores (LEGACY, V2, V3, V4 e V5) para o atual —
