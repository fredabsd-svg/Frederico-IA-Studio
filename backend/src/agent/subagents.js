...
export const subagentToolDefinition = {
  ...
  function: {
    ...
    description: 'Delega uma subtarefa AUTOCONTIDA a um sub-agente com contexto próprio, que executa ferramentas e devolve só o resultado final. GATILHO PRINCIPAL: quando o pedido tem TRÊS OU MAIS entregas independentes entre si (frentes que não dependem do resultado uma da outra), delegue as isoláveis — UMA chamada por entrega, que podem correr em paralelo — em vez de executar todas em linha nesta conversa. Também vale para subtarefa única e pesada (varrer muitos arquivos, analisar um documento longo, apurar um ponto específico, testar uma hipótese). Faça você mesmo o que é curto, o que depende do que já está neste contexto e a integração final das partes. O sub-agente NÃO vê esta conversa: escreva a tarefa completa, com todos os dados necessários.',
    parameters: {
      type: 'object',
      properties: {
