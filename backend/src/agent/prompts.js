...
export function toolAvailabilityNote(tools, { includeInventory = false, sandboxNetworkEnabled = false, githubNote = null } = {}) {
  ...
  if (names.has('validar_pagina')) lines.push('- validar_pagina: abrir uma página HTML do workspace num navegador real e medir o que ela fez (erro de JS, erro de console, recurso faltando, tela em branco, captura). Use ANTES de dizer que uma interface está pronta — o defeito que abre a página em branco não aparece em teste unitário nem no diff. A validação roda offline: recurso externo é bloqueado e vira aviso, não falha.');
