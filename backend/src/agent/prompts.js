...
- DESIGN PROFISSIONAL PRONTO: para documentos bonitos, o sandbox já tem kits testados (identidade "Tinta & Latão", mesma grade nos três) — use-os em vez de estilizar na mão: Word → \`from docpro import Relatorio\`; Excel → \`from xlspro import Planilha\`; PDF → \`from pdfpro import RelatorioPDF\`. Dão capa, sumário, títulos que numeram sozinhos, tabelas com TOTAL, listas, callouts, KPIs, citação, linha do tempo, gráficos, assinaturas, contracapa e rodapé paginado; no Excel, a aba-painel (\`p.painel\`). O \`salvar()\` do pdfpro ainda AUDITA o arquivo pronto e falha se sair com texto fora da grade ou glifo trocado. Documento registrável (ata, contrato, alteração contratual) é sóbrio: \`from docpro import Sobrio\` — justificado, sem cor. Nunca deixe placeholders ("DD/MM/AAAA", "Seu Nome"); use dados reais e a data de hoje, ou omita.
...
// Validação por navegador (Fase 38): acompanha quem ESCREVE no workspace, pela
// mesma lógica da `ambiente` — quem pode produzir uma página precisa poder
// conferi-la. Não amplia nada: a validação roda offline, contida no workspace
...
export function toolsFor(assistant) {
  ...
  if (WORKSPACE_WRITE_TOOL_NAMES.some(name => allowed.has(name))) allowed.add(PAGE_CHECK_TOOL_NAME);
...
export function toolAvailabilityNote(tools, { includeInventory = false, sandboxNetworkEnabled = false, githubNote = null } = {}) {
  ...
  if (names.has('validar_pagina')) lines.push('- validar_pagina: abrir a página num navegador real e medir o que ela fez (erro de JS, erro de console, recurso faltando, tela em branco, captura). Use ANTES de dizer que uma interface está pronta — o defeito que abre a página em branco não aparece em teste unitário nem no diff. Subiu um servidor no sandbox? Passe "porta" (o navegador roda dentro do sandbox e alcança o seu 127.0.0.1). Construiu um arquivo? Passe "caminho". A validação roda offline nos dois modos: recurso externo é bloqueado e vira aviso, não falha.');
