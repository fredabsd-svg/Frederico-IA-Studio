...
**Sub-agentes (delegação em tempo de execução).** Diferente do Modo Equipe
(`backend/src/agent/orchestrator.js`), onde os especialistas são escolhidos antes
pela interface e só opinam, aqui é o próprio agente que decide delegar: a
ferramenta `delegar_subagente` (`backend/src/agent/subagents.js`) dispara um
`runAgent` completo, com ferramentas, numa janela de contexto própria e
descartável. O sub-agente compartilha a conversa e o workspace — os arquivos que
ele gera aparecem normalmente em `outputs/` —, mas não é dono da conversa: não
